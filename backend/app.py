...
try:
    ...
    if firebase_private_key and firebase_client_email:
        ...
        cred = credentials.Certificate({
            ...
            "project_id": "expensetracker-bf1ef",
...
def create_app() -> Flask:
    ...
    if not xai_api_key:
        logging.warning("XAI_API_KEY not found in environment. Set it in .env")

    @app.route("/", methods=["GET"])
    def index() -> tuple:
        return jsonify({"status": "running", "message": "SmartSpendAI Backend"}), 200
...
Always be encouraging about their financial journey!"""
            ...
            if resp.status_code != 200:
                logging.error("xAI API error %s: %s", resp.status_code, resp.text)
